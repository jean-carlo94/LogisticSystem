...
class EventRepository(BaseRepository):
    ...
    async def get_all(self, skip: int = 0, limit: int = 100, filters: dict | None = None):
        return await Event.get_all(
            self.db, skip=skip, limit=limit, order_by=Event.create_at.desc(),
            filters=filters, tenant_id=self._tenant_id,
        )
    ...
    async def find_by_entity(self, entity_type: str, entity_id: int, skip: int = 0, limit: int = 100):
        ...
        if self._tenant_id is not None:
            base = base.where(Event.tenant_id == self._tenant_id)
