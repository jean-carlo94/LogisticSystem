...
async def list_api_keys(
    ...
    _perm: "User" = Depends(require_permission(PermissionCode.API_KEYS_VIEW)),
):
    merged = dict(filters)
    if name is not None:
        merged["name"] = name
...
async def create_api_key(
    data: ApiKeyCreate,
    service: ApiKeyService = Depends(get_api_key_service),
    user: "User" = Depends(require_permission(PermissionCode.API_KEYS_CREATE)),
):
...
async def update_api_key(
    ...
    data: ApiKeyUpdate,
    service: ApiKeyService = Depends(get_api_key_service),
    user: "User" = Depends(require_permission(PermissionCode.API_KEYS_EDIT)),
):
...
async def delete_api_key(
    key_id: int,
    service: ApiKeyService = Depends(get_api_key_service),
    user: "User" = Depends(require_permission(PermissionCode.API_KEYS_DELETE)),
):
